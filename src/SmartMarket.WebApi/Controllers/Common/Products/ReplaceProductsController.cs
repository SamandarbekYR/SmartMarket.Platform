...
namespace SmartMarket.WebApi.Controllers.Common.Products
{
    ...
    public class ReplaceProductsController(IReplaceProductService replaceProductService) : ControllerBase
    {
        ...
        [HttpPost("filter")]
        public async Task<IActionResult> FilterReplaceProductAsync([FromBody] FilterReplaceProductDto dto)
        {
            try
            {
                var replaceProducts = await _replaceProductService.FilterReplaceProductAsync(dto);
                return Ok(replaceProducts);
            }
            catch (StatusCodeException ex)
            {
