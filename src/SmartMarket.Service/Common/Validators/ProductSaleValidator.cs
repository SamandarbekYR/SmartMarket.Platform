...
public class ProductSaleValidator : AbstractValidator<AddProductSaleDto>
{
    public ProductSaleValidator()
    {
        ...
        RuleFor(ps => ps.ProductItems)
            .NotEmpty()
            .WithMessage("At least one product item is required.")
            .Must(items => items.All(item => item.ProductId != Guid.Empty && item.Count > 0))
            .WithMessage("Each product item must have a valid Product ID and count greater than zero.");

        RuleFor(ps => ps.CashSum)
            .GreaterThanOrEqualTo(0)
            .WithMessage("Cash amount must be greater than or equal to zero.");

        RuleFor(ps => ps.CardSum)
            .NotEmpty()
            .WithMessage("Card amount is required.");
